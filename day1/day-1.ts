...
//
// input.forEach(value => {
//   const sanitizedNumber: string = value.replace(/[a-zA-Z]/g, '');
//   const splitNumber = sanitizedNumber.split('');
//   const combinedNumber: number = Number(splitNumber[0] + splitNumber.reverse()[0]);
//
//
//   totalNumber = totalNumber + combinedNumber;
// });
...
const checkFirstMatch = function (value: string): string {
  for(let i= 1; i <= value.length; i++) {
    for (const numberToFormat in numberMapper){
      const number = numberMapper[numberToFormat];
      // console.log('checking if ' + numberToFormat + ' exists in ' + value.substring(0, i));
      if (value.substring(0, i).indexOf(numberToFormat) !== -1) {
        value = value.replace(numberToFormat, number);
        return checkFirstMatch(value);
      }
    }
  }
...
const checkReverse = function (value: string): string {
  for(let i= 1; i <= value.length; i++) {
    for (const numberToFormat in numberMapper) {
      const number = numberMapper[numberToFormat];
      // console.log('checking if ' + numberToFormat + ' exists in ' + value.substring((value.length - i), value.length));
      if (value.substring((value.length - i), value.length).indexOf(numberToFormat)!== -1) {
        value = value.replace(numberToFormat, number);
        return checkReverse(value);
      }
    }
  }
...
input.forEach(value => {
  const sanitizedNumber: string = checkFirstMatch(value).replace(/[a-zA-Z]/g, '');
  const sanitizedNumberReversed: string = checkReverse(value).replace(/[a-zA-Z]/g, '');
  const splitNumber = sanitizedNumber.split('');
  const splitNumberReversed = sanitizedNumberReversed.split('').reverse();
  const combinedNumber: number = Number(splitNumber[0] + splitNumberReversed[0]);
  //
  // console.log('test');
  // console.log(sanitizedNumberReversed)
  totalNumber = totalNumber + combinedNumber;
});
